...
class UserRole(YouTrackObject):
    ...
    def toXml(self):
        user_role = ElementTree.Element('userRole', name=self.name)
        projects = ElementTree.SubElement(user_role, 'projects')
        for project in self.projects:
            ElementTree.SubElement(projects, 'projectRef', id=project, url='dirty_hack')
...
class UserBundle(YouTrackObject):
    ...
    def toXml(self):
        user_bundle = ElementTree.Element('userBundle', name=self.name)
        for group in self.groups:
            ElementTree.SubElement(user_bundle, 'userGroup', name=group.name, url='dirty_hack')
        for user in self.users:
            ElementTree.SubElement(user_bundle, 'user', login=user.login, url='yet_another_dirty_hack')
        return user_bundle
    ...
    def get_all_users(self):
        ...
        for group in self.groups:
            ...
            group_users = self.youtrack.getUsers({'group': group.name})
...
class Bundle(YouTrackObject):
    ...
    def toXml(self):
        result = ElementTree.Element(self._bundle_tag_name, name=self.name)
        for v in self.values:
            result.append(v.toXml())
        return result
...
class WorkType(YouTrackObject):
    ...
    def toXml(self):
        work_type = ElementTree.Element('workType')
        attr = ElementTree.SubElement(work_type, 'name')
        attr.text = self.name

        if self.autoAttached is not None:
            attr = ElementTree.SubElement(work_type, 'autoAttached')
            attr.text = self.autoAttached

        if self.id is not None:
            attr = ElementTree.SubElement(work_type, 'id')
            attr.text = self.id
