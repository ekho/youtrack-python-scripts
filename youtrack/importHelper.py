...
def _get_custom_field(connection, cf_name):
    existing_fields = [item for item in connection.getCustomFields() if item.name.lower() == cf_name.lower()]
...
def process_custom_field(connection, project_id, cf_type, cf_name, value_names=None):
    ...
    existing_project_fields = [item for item in connection.getProjectCustomFields(project_id) if item.name == cf_name]
    values_to_add = []
    bundle = None
    ...
    for value in values:
        try:
            ...
            if e.response.status == 409:
                print("Value with name [ %s ] already exists in bundle [ %s ]" % (value.name, bundle.name))
            else:
                raise e
